...
TEST_F(TemplateURLPrepopulateDataTest, UniqueKeywords) {
  ...
  for (const PrepopulatedEngine* engine : all_engines) {
    ASSERT_TRUE(unique_keywords.find(engine->keyword) ==
                unique_keywords.end());
...
// Verifies that built-in search provider for Germany and France is Qwant.
TEST_F(TemplateURLPrepopulateDataTest, ProvidersForDEandFR) {
  // Use Germany.
