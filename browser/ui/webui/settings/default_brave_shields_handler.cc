...
void DefaultBraveShieldsHandler::RegisterMessages() {
  ...
  web_ui()->RegisterMessageCallback(
      ...
      base::BindRepeating(&DefaultBraveShieldsHandler::SetFingerprintingControlType,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
      "getHTTPSEverywhereControlType",
      base::BindRepeating(&DefaultBraveShieldsHandler::GetHTTPSEverywhereControlType,
                          base::Unretained(this)));
  web_ui()->RegisterMessageCallback(
...
void DefaultBraveShieldsHandler::GetCookieControlType(const base::ListValue* args) {
  ...
  LOG(ERROR) << "setting: " << setting << ", fp_setting: " << fp_setting;
...
void DefaultBraveShieldsHandler::GetHTTPSEverywhereControlType(const base::ListValue* args) {
  CHECK_EQ(args->GetSize(), 1U);
  CHECK(profile_);

  ContentSetting setting =
      HostContentSettingsMapFactory::GetForProfile(profile_)->GetContentSetting(
          GURL(), GURL(), CONTENT_SETTINGS_TYPE_PLUGINS, brave_shields::kHTTPUpgradableResources);

  AllowJavascript();
  ResolveJavascriptCallback(
      args->GetList()[0].Clone(),
      base::Value(setting != CONTENT_SETTING_BLOCK));
}
...
void DefaultBraveShieldsHandler::GetNoScriptControlType(const base::ListValue* args) {
  CHECK_EQ(args->GetSize(), 1U);
  CHECK(profile_);

  ContentSetting setting =
      HostContentSettingsMapFactory::GetForProfile(profile_)->GetContentSetting(
          GURL(), GURL(), CONTENT_SETTINGS_TYPE_JAVASCRIPT, "");
