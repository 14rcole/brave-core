...
bool TrackingProtectionService::ShouldStoreState(const GURL& url) {
  ...
  std::string host = url.host();
  if (std::find(first_party_storage_trackers_.begin(), first_party_storage_trackers_.end(), host) 
...
void TrackingProtectionService::ParseStorageTrackersData() {
  LOG(INFO) << "Parsing Storage Tracker Data";
  ...
  LOG(INFO) << "first_party_storage_trackers_ trackers";
  for(std::string& tracker : first_party_storage_trackers_) {
       LOG(INFO) << tracker << "\n";
  }
